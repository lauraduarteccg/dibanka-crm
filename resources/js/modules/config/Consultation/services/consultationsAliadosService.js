...
export const getConsults = async (page = 1, search = "") => {
  const { data } = await api.get(`/config/consultations-aliados?page=${page}&search=${encodeURIComponent(search)}`);
  return {
...
export const updateConsult = async (id, payload) => {
  const { data } = await api.put(`/config/consultations-aliados/${id}`, payload);
  return data;
};
