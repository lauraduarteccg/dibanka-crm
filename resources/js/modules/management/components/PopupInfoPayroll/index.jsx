...
const PopupInfoPayroll = () => {
    ...
    const infoItems = [
        ...
        },
        {
            ...
            description:
                "Afiliado solicita activación del correo institucional",
        ...
        // ACTUALIZACIÓN DE DATOS
        {
            ...
            description:
                selectedPayroll?.i_description || "Seleccione una pagaduría",
            contact: [
                {
                    type: "email",
                    value:
                        selectedPayroll?.i_email || "Seleccione una pagaduría",
                },
                {
                    type: "phone",
                    value:
                        selectedPayroll?.i_phone || "Seleccione una pagaduría",
                },
            ],
        },
        // ENROLAMIENTO
        {
            ...
            description:
                "Desea registrarse en Dibanka. Se validan los datos, autoriza a la señora a recibir la información; se le indican los pasos para el ingreso, por la opción registrarme asigna contraseña, realiza el registro biométrico culminando con un enrolamiento exitoso.",
        },
        {
            category: "enrolamiento",
            title: "Enrolamiento",
            description:
                "Desea registrarse en Dibanka. Se validan los datos, se le indican los pasos para el ingreso, por la opción registrarme asigna contraseña, realiza el registro biométrico culminando con un enrolamiento exitoso.",
        },
        // RECUPERAR CONTRASEÑA
        {
            ...
            description:
                "Afiliado se comunicó por qué necesita restablecer la contraseña, se hace validación de datos y cuenta con datos actualizados, se le indica que se va a restablecer contraseña, se procede a enviar video de WhatsApp donde le van brindar paso a paso para el restablecimiento de contraseña",
        ...
        },
        {
            ...
            description:
                "Afiliado indica que desea restablecer la contraseña de Dibanka. Se validan los datos, autoriza a la señora _____________ a recibir la información; se le indica que se transferirá con un audio que le indicara el paso a paso para el restablecimiento",
        },
        {
            category: "contra",
            ...
            sections: [
                {
                    subtitle: "CASUR",
                    description:
                        "Afiliado se comunicó por qué necesita restablecer la contraseña. Se validan los datos, se evidencia desactualizados, se le informa que debe solicitar el formato de actualización al correo citse@casur.gov.co una vez cuente con el formato, lo diligencia y lo envía al correo que le acabo de indicar junto con la copia de su cédula.",
                    contact: [{ type: "email", value: "citse@casur.gov.co" }],
                },
                {
                    subtitle: "FPS",
                    description:
                        "Pensionado se comunicó por qué necesita restablecer la contraseña. Se validan los datos, se evidencia desactualizados, se le informa que debe actualizar datos, enviar un correo electrónico a correspondencia@fps.gov.co indicando nombre completo, cédula, celular, correo electrónico, dirección, ciudad y departamento; se le sugiere comunicarse cuando cuente con N° radicado.",
                    contact: [
        ...
        },
        {
            ...
            description:
                "desea asesoría para el ingreso de la solicitud. Se validan los datos, se le sugiere ingresar a la plataforma, se le indican pasos para el ingreso, realiza el registro biométrico y se culmina el ingreso de la solicitud de forma exitosa.",
        },
        // BLOQUEOS
        {
            category: "bloqueos",
            ...
            sections: [
                {
                    ...
                    description:
                        "indica que presenta un bloqueo por descuento en cola y desea validar la razón. Se validan los datos, se le informa que es con la entidad __________ debe comunicarse con ellos para llegar a un acuerdo y se pueda retirar el bloqueo. ",
                },
                {
                    subtitle: "envío a soporte - ticket en validación",
                    description:
                        "indica que presenta un bloqueo por descuento en cola, envió la solicitud a soporte y desea validar una respuesta. Se validan los datos, se le indica que la solicitud fue recibida bajo el N°  esta se encuentra en validación nos comunicaremos cuando contemos con una respuesta.",
                },
                {
                    subtitle: "en validación",
                    description:
                        "indica que tiene un bloqueo por descuento en cola y desea validar si ya tenemos una respuesta. Se validan los datos, se le informa que la solicitud está en validación bajo el N° ____ nos comunicaremos cuando contemos con una respuesta. ",
                },
            ],
        },
        {
    ...
    const categories = [
        ...
        {
            id: "credito",
            name: "Solicitud crédito",
            icon: <MdAttachMoney size={18} />,
        },
        {
            id: "bloqueos",
            name: "Bloqueos",
            icon: <PiWarningCircleLight size={23} />,
        },
    ];
    ...
    const tickets = [
        {
            type: "afiliados",
            message:
                "Se intentó generar comunicación con el afiliado en varias ocasiones y se va a buzón de voz.",
            action: "Por favor comunicarse a la línea 6012193096 para validar su solicitud.",
        },
        {
            type: "operador",
            message:
                "Se intentó generar comunicación con el Operador en varias ocasiones y se va a buzón de voz.",
            action: "Por favor comunicarse a la línea 6012193096 para validar su solicitud.",
        },
    ];
    ...
    const filteredItems = (infoItems || []).filter((item) => {
        ...
        const matchesSearch =
            ...
            (item.description &&
                normalizeText(item.description).includes(searchNormalized)) ||
            (item.action &&
                normalizeText(item.action).includes(searchNormalized)) ||
            normalizeText(item.category).includes(searchNormalized) ||
            (item.items &&
                item.items.some((subItem) =>
                    normalizeText(subItem).includes(searchNormalized)
                )) ||
            (item.contact &&
                item.contact.some((c) =>
                    normalizeText(c.value).includes(searchNormalized)
                )) ||
            (item.sections &&
                item.sections.some(
                    (section) =>
                        normalizeText(section.subtitle).includes(
                            searchNormalized
                        ) ||
                        normalizeText(section.description).includes(
                            searchNormalized
                        ) ||
                        (section.contact &&
                            section.contact.some((c) =>
                                normalizeText(c.value).includes(
                                    searchNormalized
                                )
                            ))
                ));
                            ...
                            {item.items && (
                                <ul className="space-y-2">
                                    {item.items.map((subItem, subIndex) => (
                                        ...
                                        >
                                            <span className="text-purple-500 mt-1 font-bold">
                                                ▸
                                            </span>
                                            <span>{subItem}</span>
                                        </li>
                            ...
                            {item.sections && (
                                <div className="space-y-4 mt-3">
                                    {item.sections.map((section, secIndex) => (
                                        ...
                                        >
                                            <h4 className="font-bold text-purple-700 mb-2 text-lg">
                                                {section.subtitle}
                                            </h4>
